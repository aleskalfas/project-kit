...
- **Latent extraction (COR-007, not now).** The per-arm dispatch ladder ("which binding am I in → what to demand") is still hand-written in each gating consumer; a *third* such consumer is the trigger to extract a single disposition→(read, presence-demand) resolver — the shape-reading predicates already prevent read-drift, but not ladder-drift. Extract on the third, not the second. (This is a *code-extraction* threshold — distinct from the *pattern-promotion* question ADR-042 / DEC-022 settle at the second instance: this record likewise declines to promote the disposition/severity rule to a core-level COR, staying an ADR alongside 042/038.)
