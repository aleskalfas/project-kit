...
1. **`candidates`** — threaded with the contract's upstream process address and trigger state; yields the upstream candidate subject ids (the analogue of cascade's `members`). Each candidate's position at the trigger is then confirmed **one subject at a time** through the **engine's existing single-subject position resolution** — the source proposes, live detection disposes.
   ...
   > **Realization note (implementation, #610).** The reused-unchanged runner passes exactly one variable input (the subject positional), so threading both address *and* trigger is impossible without changing the runner — which this record's Implications rule out. The implementation threads the **upstream process address in the subject slot** (a deliberate reinterpretation of the slot) and leaves the trigger to the binding's static knowledge of its own contract declaration. Safe by this record's own design: the source only *proposes* — the engine's at-exact-trigger confirm disposes — so a passed trigger would only ever have been a narrowing hint; over-proposal is filtered by the confirm, and under-proposal is the named authoring smell either way. A `candidates` command shared across contracts with different triggers may therefore over-propose freely; health filters.
