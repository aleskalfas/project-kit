---
name: report-author
description: Help a user compose and file a well-formed bug or feedback report to the upstream project-kit repo via `pkit report`. Use when someone wants to report a problem or share feedback about pkit and would benefit from an agent drawing out a clear, actionable description.
metadata:
...
`pkit report` is the built-in adopter→project-kit feedback channel (per the report-command decision, PRJ-008; cross-repo realization ADR-047). It files a **bug** (something is broken) or **feedback** (an idea, friction, or rough edge) as an issue on the upstream project-kit repo — *not* the adopter's own tracker — with a redacted environment block attached automatically.
...
## 1. Pick the kind: bug vs feedback
...
- **feedback** — a rough edge, a missing affordance, a confusing name, an idea. There may be nothing "broken"; the value is the signal.

When unsure, default to **feedback** — it carries no repro burden and is easy to reclassify.
