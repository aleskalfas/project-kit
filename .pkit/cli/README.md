...
| `upgrade` | version-aware migrations + sync; in a pinned project, auto-advances the `.pkit/version-pin` directive to the latest release (reconcile forward via `uvx`, flip the pin last; no `uv tool install`) per ADR-049. `--pin` also pins an *un-pinned* project at the version it upgrades to (opt-in one-step lock; offline-safe; no-op if already pinned) | yes | yes |
...
**Opting an un-pinned project in with `--pin`.** An un-pinned project stays un-pinned after `upgrade` by default (it keeps following the global tool). Pass **`--pin`** to also pin it, in one gesture, at the version it just upgraded to — the "I upgraded, so lock to what I upgraded to" shortcut that saves the separate `pkit pin` step. It pins at the *local* synced version (offline-safe — no `git ls-remote` latest lookup), writes the pin **last** (after content + migrations, so a failed sync never leaves a pin ahead of content), and is a **no-op on an already-pinned project** (the auto-advance above already maintains the pin). Plain `upgrade` is unchanged, so `--pin` never silently switches a project into the pinned (uvx-re-exec) execution model — it is always an explicit opt-in. (Making *new* projects pinned by default is a separate, larger question tracked on EPIC #626.)
