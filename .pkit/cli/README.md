...
| `permissions setup autonomy [--profile <name>]` | goal-oriented setup (first ADR-007 instance): stand up autonomous agents by composing `profile activate` + `enable` + `sandbox enable`, auto-resolving the SSH-agent socket (`$SSH_AUTH_SOCK`, per ADR-010), stop honestly at the session-restart boundary, and on re-run verify via the probe suite — the goal is declared reached only when the proof passes (decision layer + credential floor). **Auto-applies the one platform-mandatory, necessity-verified required exclusion** (per ADR-027): on macOS with a uv repo marker and an installed `uv` at/below the known-bad release (the SystemConfiguration Seatbelt panic, ADR-014), it excludes `uv` (and `pkit`, which runs via `uv run`) through the real `sandbox exclude` primitive under a distinct `_required` provenance tag — loud (the UNCONFINED banner), in its own setup block, reported by `sandbox status` as auto-applied, never written to a committed file. A fixed `uv` self-disables it. Optional widenings stay in the **NEXT** block of explicit gestures it detects but won't run for you — `gh` exclusion (widening) and commit-signing socket (`accommodate --socket`). **Self-heals**: a later run removes a previously auto-applied `_required` exclusion once the version floor shows it's no longer required (uv upgraded past a fix / on Linux), never touching an operator's `_manual` carve-out. Stepwise, resumable; no dangerous-flag pass-through | yes | no — resumable + idempotent (live system is the checkpoint) |
