...
| `pin [<version>]` | write the `.pkit/version-pin` directive (per ADR-045): no argument freezes at the current content version (`backbone_version`); `<version>` (a version number only, leading `v` stripped) freezes (equal), reconciles content forward then flips the pin last (newer), or refuses (older — forward-only migrations). Requires the manifest; refuses branch/sha/pre-release pins. Project-owned; never kit-synced | yes | no — overwrites an existing pin |
