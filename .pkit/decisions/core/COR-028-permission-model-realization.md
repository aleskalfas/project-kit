...
3. **Unenforceable intent is reported, not dropped.** A realizer declares which dimensions of the model its harness can natively enforce. When the model expresses intent the harness cannot faithfully enforce, the realizer renders the closest achievable enforcement and **reports the residual gap** rather than silently discarding it — so the adopter can close the gap outside the harness (for example at the operating-system, container, or network layer). *(Scope note, added with [COR-045](COR-045-agent-invocation-contract.md): this rule governs **permission** realization, where a partial boundary still constrains and the gap report reaches an adopter who can act on it. Another realizer domain may need the opposite answer — refuse rather than degrade — where neither of those holds; COR-045 states the discriminating test and applies it to agent-invocation transports. Nothing in this rule changes.)*
