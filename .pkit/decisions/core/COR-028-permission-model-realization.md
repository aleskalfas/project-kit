...
- **Point 3 is the permission-domain instance of a general rule.** [COR-047](COR-047-harness-requirement-declaration.md) generalises the realizer-declares-and-reports rule to every property the methodology requires of its harness: requirements are declared in behavioural terms, each adapter declares its support, and a requirement that is undeclared or unobservable is reported as indeterminate, never satisfied. This record's decision points are unchanged; point 3 is read as that general rule applied to permissions.
