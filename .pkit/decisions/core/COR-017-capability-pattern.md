...
> **Refined by [COR-031](COR-031-capability-origin.md).** The lifecycle below assumes a capability is copied from kit source on install (origin `kit-shipped`). A capability *authored in the adopter's own repo* (origin `incubated-in-repo`) is registered in place — no copy — and excluded from source-reconciliation by sync/upgrade, since there is no kit source to reconcile against. See COR-031 for the origin axis.
