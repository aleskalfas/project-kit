...
- **The binding supplies the set; the engine folds.** The set folded over is the keyed subjects of the **one** named child process that belong to the given parent subject. The engine does **not** hold or discover a containment tree — it asks the binding through a **two-predicate seam**: a **`members` source** (run once, threaded with the *parent* subject) yields the parent-scoped candidate ids — the sole parent-scoping authority — and a per-candidate **`membership` test** (run one subject at a time by the existing single-subject runner, which carries *only* the candidate, never the parent) re-confirms each candidate from its own reality, the same content-free seam COR-033 uses for detection. The set is whatever the seam returns **at each evaluation**, re-read live — determinate at the instant, never a stored or open-ended listing. (Because the engine never threads the parent to the `membership` test, parent-specificity lives in `members`; the per-candidate test does liveness + the indeterminacy fail-closed, not re-scoping. Engine-strategy detail: ADR-023.)
