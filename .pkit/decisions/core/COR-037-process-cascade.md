...
- **Fold semantics are fail-closed.** A member whose outcome is not yet resolved (still moving, parked, indeterminate) holds the fold **unresolved** — the parent stays blocked, never reads a false "all reached X." The **empty set** (a parent with no children of that process yet) is **fail-closed too**: `all` over no members does **not** vacuously open the gate — an area with zero points-of-interest is not "fully discovered."
