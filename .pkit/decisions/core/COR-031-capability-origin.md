...
- **Deferred work.** The external-capability-source origin's distribution and reconciliation are now **realised by [COR-041](COR-041-external-source-distribution.md)** (consumed-whole, pinned, match-the-pin — with the write-path in ADR-040). Incubated→kit-shipped graduation remains reserved as a future transition; it earns its own decision when a consumer exists.
