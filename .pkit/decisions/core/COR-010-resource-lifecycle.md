...
- **Capability origin gates reconciliation** ([COR-031](COR-031-capability-origin.md)). The Update / reconciliation step skips a capability authored in the adopter's own repo (origin `incubated-in-repo`) — it has no kit source to reconcile against and is adopter-owned per the no-shared-files invariant. Such a capability still participates in compatibility resolution, dependency gating, and deploy; only source-reconciliation is suppressed.
