...
`uv tool install` is the **sole sanctioned frontend** (per PRJ-003) — `uv` is a hard requirement, not merely recommended. The former `pip install git+...` fallback is **dropped**: the router hard-assumes `uvx` for pinned re-execution ([ADR-039](../../../docs/architecture/decisions/ADR-039-pkit-entry-point-router.md)), so a `pip`-installed pkit is a latent inconsistency, and self-service tool-update ([ADR-044](../../../docs/architecture/decisions/ADR-044-upgrade-self-update-detect-instruct.md)) instructs a `uv` command; supporting a second frontend buys nothing at this project's single-digit-adopter, uv-native scale and only adds a "which pkit am I running" ambiguity. This narrows the *frontend* set, not the channel (still a direct git URL, no registry — so this is an in-place amendment, not a supersession). The HTTPS form (`git+https://github.com/...`) remains supported for users who authenticate to GitHub via PAT instead of SSH.
...
- **`pyproject.toml` at the repo root** (per PRJ-003) is required for git-URL install to work. The `[project]` table declares the package name (`project-kit`), version (read from `.pkit/VERSION` or via dynamic versioning), and entry point (`pkit = "project_kit.cli:main"`). **The built wheel also bundles the methodology content as package data, so the installed binary can `init`/`sync`/`upgrade` without a checkout. What it carries is defined by *tier ownership* — the kit-owned tree, with the *contents* of the source project's own `project/` tier withheld — so no adopter receives another project's config, activation switches or audit journals. Those directories ship as empty markers, because the installer reads the bundle's shape to decide what to scaffold. See [ADR-033](../../../docs/architecture/decisions/ADR-033-official-install-bundles-content.md), which clarifies (does not supersede) this implication.**
...
- **Release notes ship as a notes-only GitHub Releases overlay** (the option foreseen here, realized 2026-07-05). `pkit release publish-notes` publishes each version's `CHANGELOG.md` section as the Release body, wired into the tag workflow. It is **notes only — never an attached artifact**: a wheel/tarball would reintroduce the release-artifact channel this record rejects, so attaching one stays forbidden and the install path is unchanged (still the git tag, `uv tool install …@vX`). This realizes a foreseen implication without overturning the decision, so it is recorded here, not superseded (per the channel-change note below).
...
- **Adopter doc** (`.pkit/cli/README.md`'s "Installing pkit on PATH" section) graduates from the bootstrap symlink-pattern to `uv tool install` once the Python runtime ships per the build roadmap. With the uv-only narrowing above, `uv` is a prerequisite — there is no "machine without `uv`" supported path.
