...
2. **Capture lives in the adapter, not the core.** The "prompt" is a harness behaviour, so capture is written in the Claude-Code adapter hook *after* the decision is computed, gated on the deferred verdict, and wrapped so a capture failure can never change a decision or break fail-open. The shared decision core stays pure (it is the same code the CLI runs). The classifier, taxonomy, and report stay harness-agnostic in the permissions surface. Because the hook observes only its own deferral — not whether the harness ultimately prompted — the captured signal is a *superset* of real prompts; the report states **coverage**, not a predicted prompt-count decrement. The report later *sharpens* this superset with two static axes over the logged command text — **prompted vs auto-allowed** (flat-matching the whole command against the live `settings.json` allow patterns) and, of the real prompts, **allowlisted-but-compound vs genuinely-missing** — but the axes only refine the coverage framing; they still promise no live prompt-count decrement, and they remain recommend-only (the mechanics live in code + the permissions README per sub-decision 6, not in this record).
...
- **Privacy footprint.** The diagnostic log is a new local artifact carrying redacted command strings; it is git-ignored via the pkit-owned `.pkit/.gitignore` render (per ADR-009 rule 7) and size-capped. The redaction-on-by-default and ignore must ship with capture, not after.
...
- **The report's classification axes** (prompted vs auto-allowed; allowlisted-but-compound vs genuinely-missing, issue #317) are likewise inventory: they live in the permissions surface code and the permissions README, not here, consistent with sub-decision 6. They reuse the existing settings readers, the `Bash(…)` allow-pattern parser, and the canonical shell-operator splitter, so they introduce no new matcher and no capture-format change; they keep the recommend-only bright line intact.
