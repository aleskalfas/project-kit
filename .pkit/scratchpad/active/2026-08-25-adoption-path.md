...
### Architect verdict on the init-resolution redesign (ADR-001)
- **Amendment, NOT a new ADR / not a supersession.** ADR-001's spine survives (implicit-from-cwd, git-first-for-identity, fallback-because-pkit-doesn't-require-git). The redesign *refines* the fallback (validate candidates) + the return-contract *implication* (init gets a classified projection; shared resolver stays `Path | None`). Per the decisions-README "Refining an accepted record," that's an in-file **Amendment 1** (ADR-009 precedent). Resolves critic's W1 (we validate the fallback, not delete it). **Authorisation gate (low ceremony): maintainer accepts Amendment 1, landed in the SAME change-set as the implementation.**
- **Design centre — ONE subprocess-free structural validator, two altitudes:**
  - *Correctness floor (shared, low):* "is this `.git` a real repo?" — a subprocess-free structural predicate (`.git/` dir with `objects/`+`refs/`, or a valid `gitdir:` `.git` file). Rejecting a broken `.git` is universally correct → lives in `find_target_root` (→ `Path | None`); benefits sync/status/upgrade too; works even with git absent (preserves the no-git guarantee).
  - *Init consent policy (high):* the guided classification (install-here / git-subfolder→root / dubious-ownership→**detect+guide** / pkit-install→**refuse+redirect to sync**) lives in `resolve_init_target` ONLY — must NOT leak into the ~150 read-only call sites.
  - **Extract one private helper (git-first + walk-up + structural-validate); both resolvers become thin projections** — discharges the COR-007 resolver-extraction item (L158) in the same move.
- **git-stderr (Q4):** classify on structure + exit-128, NEVER on stderr prose (locale/version-fragile); stderr may only enrich the guidance message.
- **Router (Q2):** intentionally left on bare `.git` for now — benign by construction for broken-`.git` (washes to run-self); router can't take `git -C` (ADR-039 no-subprocess hot path). The shared structural validator is exactly what the eventual 3-resolver reconciliation needs. Keep this axis separate from the nearer-`.pkit`-wins split-brain (owned by the deferred monorepo ADR).
- **Doc-drift caught:** ADR-001 is ALREADY stale on `main` (#780 shipped `resolve_init_target`/`InitTargetReason` but ADR-001 still says "Commands receive `Path | None`"). Amendment 1 reconciles both #780's split AND this validation refinement in one gesture.
- **Custody:** architect DRAFTED the full ready-to-commit Amendment 1 text but did NOT commit it (machinery is on `main`, not this branch; committing an accepted amendment for unshipped behaviour = the F7/F8 forward-drift anti-pattern). Lands off `main`, in the implementation PR. Enumerate #780's existing init tests so the redesign doesn't regress them.
