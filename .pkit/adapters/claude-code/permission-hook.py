...
`load_model`, so they can never diverge.

Fail-OPEN (ADR-002): any fault — unreadable model, import failure, malformed
payload — yields a silent abstain (exit 0, no stdout), never a silent block.
The non-negotiable guardrail denies are double-locked in the harness's
fail-closed native `settings.json` denies, so failing open here can never
bypass them. Set PKIT_PERMISSIONS_DEBUG=1 to surface fault reasons on stderr
