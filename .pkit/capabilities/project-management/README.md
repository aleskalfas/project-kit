...
All seven are idempotent at the level of observable state — re-running after a partial failure recovers cleanly. Audit comments use DEC-024's template-stamp markers (`<!-- pkit-hook: <name> -->`) so re-posts detect existing entries and skip — except the **canonical audit line** itself, which since [project-management:DEC-049-audit-model] carries the uniform `<!-- pkit-audit -->` marker and is rendered from one schema template (`move-issue`, and `done-work`'s per-reviewer override, both go through `scripts/_lib/audit.py`).
