...
Both `remote_registered:` and `local_registered:` are optional lists. `remote_registered:` is still capped at one entry at v1 (per DEC-028); **`local_registered:` is not** — [project-management:DEC-032-reviewer-contribution-socket] D3 lifted that cap, and N ≥ 2 local reviewers is valid (`pre-check` shape-validates every entry). Each entry contributes a path to the agent gate's OR composition, and the baseline names are unioned with any capability-contributed reviewers to form the PR's required set.
