...
Both `remote_registered:` and `local_registered:` are optional lists. `remote_registered:` is still capped at one entry at v1 (per DEC-028); **`local_registered:` is not** — [project-management:DEC-032-conditional-reviewer-requirements] D3 lifted that cap, and N ≥ 2 local reviewers is valid (`pre-check` shape-validates every entry). Each *populated list* contributes a path to the agent gate's OR composition; **within** the local path every registered reviewer must approve (all-must-approve, per DEC-028's multi-local table — `done-work`'s gate checks that every required reviewer has a fresh APPROVED). The baseline names are unioned with any capability-contributed reviewers to form the PR's required set.
