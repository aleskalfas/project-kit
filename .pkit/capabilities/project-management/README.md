...
When the agent-mode gate passes, `done-work` prints a **per-reviewer disposition summary** so a green gate can never imply every perspective reviewed the code (bug #715: `APPROVED` reads as "reviewed" even when a slot was only overridden). It enumerates each required reviewer in the resolved set as exactly one of:
...
A closing **honesty line** counts and names every perspective that did *not* genuinely review (or confirms all did). Sample:
...
```
  ...
  honesty: 2 of 3 required perspective(s) did NOT genuinely review this PR (satisfied by override): local agent (design-reviewer, …), local agent (security-reviewer, …)
```
