...
**`show-tree --limit` bounds the view, and a bounded view says so.** The flag is a deliberate view control for a large tracker and keeps its default. What changed is honesty about the result: the tree is built from the seam's corpus, which reports whether it was enumerated to exhaustion, so a render that struck the limit is labelled `[partial]` — on stdout *and* stderr in text (so a redirected render keeps the caveat), as a `> **Partial view**` note in markdown, and as `"complete": false` in JSON. Without that a reader cannot tell "this parent has no other children" from "I stopped looking" — and this is the diagnostic people use to judge whether a container is ready to close. A complete render carries no notice, so the label means something.
