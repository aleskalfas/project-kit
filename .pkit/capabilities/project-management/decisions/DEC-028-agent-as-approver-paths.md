...
review:
  agents:
    ...
    local_registered:
      - name: pm-reviewer                 # matches `.claude/agents/pm-reviewer.md` (default, shipped by this capability)
...
- **Local-path default reviewer agent.** The pm capability ships a default `pm-reviewer` agent at `.pkit/capabilities/project-management/agents/pm-reviewer.md` (per COR-026 — discipline-implying agents live in the capability that ships the discipline). Adopters who configure `local_registered: name: pm-reviewer` get the kit-shipped default; adopters may also replace or add to it with their own agents under `.claude/agents/` (subject to v1's singleton-per-path constraint). The reviewer agent's body emits the local-path verdict format and applies the capability's conventions (Conventional Commits, branch/type alignment, classification axes, surface-change discipline). The remote path ships no default — bot identity, GitHub App, or service-account setup is adopter-managed. The methodology specifies the contract; the local-path agent implementation is now kit-shipped via this capability, the remote-path agent implementation is adopter-side.
...
- **Acceptance gate** (per `.pkit/rules/core.md` rule 2): this DEC depended on **DEC-026 and DEC-027 accepting first** — the cite-graph is one-directional (this DEC depends on both, not vice versa). All three are now `accepted` and the implementation (the gate-check algorithm, the config schema fields, the refusal template, `review-pr.py`) has shipped; this bullet stands as the record of that satisfied dependency.
...
**`review-pr` now also delivers the verdict as a NATIVE GitHub review — not only the comment sentinel.** This is the native migration this DEC named as future work ("future migration from comment-based sentinels to actual `gh pr review` objects is mechanically straightforward"), prompted by a dogfooding adopter (mockingbird, on GitHub Enterprise) where the comment verdict did **not** satisfy branch-protection "required approving reviews" and an agent had to hand-post a native review. Status unchanged (`accepted`); the comment sentinel, the gate-check algorithm, identity-match, author-exclusion, and freshness all stand — native delivery is **added alongside** the comment, not a replacement.

**What changed.** After posting the verdict comment, `review-pr` posts a **native `gh pr review`** carrying the state — `APPROVED → --approve`, `CHANGES_REQUESTED → --request-changes` — with the same verdict body. The two signals compose: the **comment** (with the `<!-- pkit-verdict -->` marker) remains what `done-work`'s gate reads (unchanged), while the **native review** is the GitHub-facing state that shows in the PR UI and satisfies branch protection. `--no-native` opts out (comment-only, the prior behaviour).

**Self-approval degrade — the reason this DEC originally chose comments.** GitHub refuses a native review (approve *or* request-changes) on your **own** PR. On the local path `review-pr` posts under the developer's own credentials, so when the **reviewer identity == the PR author**, the native review is **skipped** (the comment verdict stands). Native delivery is therefore fully autonomous only on a **non-author / bot identity** (the remote path). Any other gh failure (missing review permission, API error) likewise degrades to comment-only — native delivery never fails `review-pr`. This is exactly the constraint the original "why a comment, not a review" rationale named; the amendment doesn't defeat it, it delivers native review **where GitHub permits it** and degrades where it does not.

**Deferred.** A `native`-**only** delivery mode (drop the comment; make `done-work`'s gate read the native agent-review state directly) is deferred — the current `both`-by-default keeps `done-work` unchanged (it reads the comment) while giving GitHub the native state, which fully solves the adopter's branch-protection gap. The gate already accepts a native APPROVED review via the human path; extending the agent path to native-only is a follow-up if a comment-free flow is wanted.

## Amendment (2026-08-25)

**The default local reviewer agent was renamed `reviewer` → `pm-reviewer` (#770).** The single-token name `reviewer` collided with the English role-noun "reviewer" that saturates this capability's schemas and prose (required reviewers, reviewer-contribution rules, per-reviewer overrides), making the *registered agent name* indistinguishable from the *role* it plays. The rename disambiguates: the shipped default is now registered as `pm-reviewer`, deployed at `.claude/agents/pm-reviewer.md` from the canonical `agents/pm-reviewer.md`. Status unchanged (`accepted`); this amendment only rebinds the default `<name>` value — the config examples and the Implications "Local-path default reviewer agent" bullet above already read `pm-reviewer`.
