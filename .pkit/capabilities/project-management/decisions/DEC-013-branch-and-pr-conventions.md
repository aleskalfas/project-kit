...
- **Read-side recognition.** Because the marker sits *above* the parent-ref, every parent-ref *read* recognizer skips a leading `Integration: integration/<slug>` line before reading the parent-ref off the first content line — so a marked descendant validates, walks its parent chain, and renders in the tree exactly as an unmarked one. The skip is a single shared helper (`strip_integration_marker`) consumed by validate-issue, create-issue, edit-issue, move-issue, close-issue, containment, show-tree, show-issue, and the inference layer. The helper's marker pattern is kept byte-identical to `body-format.yaml`'s `integration_marker.pattern` (the schema that owns the marker's shape; `git-conventions.yaml`'s `designation_marker_pattern` is its git-side view), so a line that does not match the strict `integration/<slug>` form is *not* skipped. validate-issue turns that into a precise `body.integration-marker` hard-reject naming the expected form — rather than the misleading `body.parent-ref` message the malformed line would otherwise trigger by falling through — which is the intended guard against typo'd markers. The *write* path that inserts a parent-ref (`set-field --parent`) must likewise preserve the marker's position; that is tracked separately (#765).
...
- **Cascade and closure.** Marked Tasks close on merge into `integration/<slug>` via the normal `Closes #N` keyword. The owning issue becomes eligible-to-close per [project-management:DEC-006-state-machine-and-cascade] when every marked descendant closes. Final closure is a separate `integration/<slug>` → `main` PR with `Closes #<owning>` — squash-merged. The integration branch is deleted with that merge.
- **Squash invariant retained at both hops.** PRs into the integration branch and the final PR into `main` are both squash-merged with the head branch deleted on merge. One PR → one commit, applied uniformly.
