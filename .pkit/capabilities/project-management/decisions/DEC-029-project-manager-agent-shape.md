...
**Invocation pattern: parent-mode by default; subagent-mode dispatches until the depth limit.** The dispatch language above applies in full when `project-manager` is the **parent session** — booted via `claude --agent project-manager` or via the default-agent toggle the project-management capability ships (per [DEC-030](DEC-030-capability-contributed-adapter-overlays.md)). Claude Code's current documentation ([sub-agents.md](https://code.claude.com/docs/en/sub-agents.md)) states that "by default, a subagent can spawn subagents of its own, up to three layers below the main conversation"; the depth is set by `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` (default `3`; `1` turns nesting off), and the `Agent` tool is withheld from a subagent only **at the depth limit**, even when listed in its `tools:`. So when `project-manager` is invoked as a subagent of another session (e.g. via `Agent({subagent_type: "project-manager"})`), it **can** dispatch reviewers itself unless it sits at the limit — in which case it surfaces the *recommendation* ("reviewer X should run on this arc") and the outer session dispatches. Parent-mode remains the intended pattern for adopters: it guarantees the reviewer stack its full depth, keeps the `Agent` tool present, and keeps dispatch and the approval gate in one session. Whether the methodology should itself gate nesting depth is a separate policy question owned by the `2026-07-02-agent-dispatch-privilege-model` scratchpad note; this record takes no position on it.

**Dispatch discipline: helpers are dispatched without a name.** When `project-manager` invokes any helper through the `Agent` tool it passes no `name`. Under Claude Code's experimental agent-teams feature a named spawn is promoted to a persistent teammate whose result reaches the dispatcher truncated at a fixed size, with the remainder reachable only through a messaging tool this agent's grant does not include — and reviewer output is several times that size, so most of a review is lost while the surviving prefix reads as complete. This is the behaviour the harness requirement `delegated-result-complete` protects — an entry in the core `harness-requirements` schema, per [COR-047](../../../decisions/core/COR-047-harness-requirement-declaration.md): a delegated helper's full result reaches the agent that delegated to it. The rule is stated as a **capability check, not a remembered convention**: if the `Agent` tool exposes a `name` parameter, teams is enabled and the hazard is live; if it does not, naming is not expressible; the correct action is identical either way. It is **standing discipline**, not an interim measure: a harness-level report can detect that teams is enabled, but only the acting agent, at the moment of dispatch, can decline to name a spawn. The agent body carries the operational form of this paragraph.
