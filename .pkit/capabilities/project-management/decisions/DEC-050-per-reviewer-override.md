...
4. **Audited, off the verdict stream.** Before the merge it posts a bypassable-with-audit comment (DEC-014 shape) recording: the overridden reviewer, the operator (name + email), the reason, **and the reviewer's state at override time** (`none` / a fresh `CHANGES_REQUESTED` / a stale `APPROVED`) with a pointer to the block comment if one exists. The comment is **prose, verdict-grammar-distinct** — no `Reviewer agent:` first line, no `<!-- pkit-verdict -->` marker — so neither the gate's verdict reader (DEC-047) nor ADR-042's read surface ever mistakes it for a verdict. A per-reviewer(+reason) idempotency stamp lets a re-run overriding a *different* reviewer, or the same reviewer with a *different* reason, post its own audit.
...
**Amends [project-management:DEC-032] D3 in place** (the satisfaction predicate gains the `satisfied-by-override` OR-branch) and **extends [project-management:DEC-046]** (the parameterised bypass-family member), per the precedent DEC-032 set amending DEC-028 step 7 — both land when this DEC is accepted.

## Rationale

**Why per-reviewer, not whole-gate only.** Under a panel, one false block otherwise forces `--bypass` — discarding *all* review and training the blanket-bypass reflex. A per-reviewer override waves off the one, keeps the rest gating, and is *more* legible (per-reviewer name + reason vs one blanket reason). It is the safety valve DEC-002's block-only-on-objective discipline pairs with — an LLM reviewer is not perfectly tunable, so a per-PR valve is necessary however well thresholds are set.

**Why `satisfied-by-override` must be a distinct, first-class state.** DEC-002's third companion — the honest gate summary — must show "*N reviewed, 1 overridden*", not "*N approved*", or it re-creates #715's false-green from the other side. That requires the override to be a recorded state the gate-checker distinguishes, **not** a fabricated `APPROVED` comment (which would corrupt the verdict record and ADR-042's permissive read surface). Hence a separate gate-checker input + a verdict-distinct audit comment.
...
- **Whole-gate `--bypass` only** (status quo). Rejected — one false block discards all review and trains the blanket-bypass reflex (the DEC-002 concern).
...
- **Non-overridable ("hard") reviewers** (e.g. an un-overridable `security-reviewer`). Rejected — reintroduces the veto that trains the blanket bypass; DEC-002's philosophy is block-only-on-objective *plus* an always-available valve. Uniform overridability.
...
- **Amends DEC-032 D3 in place** and **extends DEC-046** (both amended when this DEC is accepted, per the DEC-028-step-7 precedent).
- **Depends on the honest gate summary** (DEC-002 companion (c), a sibling Task) to *render* `satisfied-by-override` distinctly — this DEC produces the state; that Task surfaces it. Shipped without it, the override still audits but the summary line is not yet honest.
- **Grammar dependency:** the overridden state's fidelity rests on a reviewer's block being machine-legible — the block token is `CHANGES_REQUESTED` (DEC-028; DEC-002 corrected to match). There is no "override a `REJECTED`" case, only "override a fresh `CHANGES_REQUESTED`" vs "override an absent verdict."
...
- Stands on DEC-028, DEC-032, DEC-014, DEC-046 (all accepted) and realises the DEC-002 companion.
