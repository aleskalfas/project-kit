...
> **Extended by [DEC-035](DEC-035-instance-ownership.md).** When instance ownership is active, `handoff-issue` gains `--to-instance <N>` (hand to another of the same person's clones), `--to-instance self` (reclaim to this clone), and `--recursive` (whole subtree). In the same-person multi-clone case the assignee is unchanged and only the `instance:N` ownership label flips — a transfer mode this record's original cross-person assignee-change form did not cover. Refinement-in-place, not a supersession: the cross-person handoff defined here stands unchanged.
