...
- Adopting projects may optionally define code-path → doc-path mappings in `project/config.yaml`, checked by `check-doc-mapping` (read-only). `enforce: false` (default) reports would-fire mappings as advisory; `enforce: true` makes a *mapped-code-without-mapped-doc* PR a hard-reject. Per [ADR-019] the capability ships this *mechanism*; the **real boundary** is the adopter wiring `check-doc-mapping` as a required CI status check (the `merge-pr` layer is only a speed-bump, per ADR-004). It stays **opt-in / default-off** — the "adopter may upgrade" latitude this record's severity table grants, *not* a mandate (mandatory/default-on is rejected: thin-docs adopters can't define sound mappings — COR-014). A `## Doc impact` line naming the code path overrides a false positive (bypassable-with-audit). Use **surgical 1:1 couplings** — broad `tree/** → README` rules false-positive on most PRs (per the dry-run audit), so keep those `enforce: false`.
