...
# ]
# ///
"""Project-management capability — detect-todo (process predicate, DEC-033).

Detection predicate for the 'todo' lifecycle state. Resolves the issue's live position via move-issue's exact inference precedence (closed->done; first state:* label; milestone->backlog; else todo) and reports result=True iff it equals 'todo'. State meaning: Filed; not yet scheduled to a Milestone.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Detect whether an issue is in the 'todo' lifecycle state.")
    ...
    if not bootstrap_gate.enforce("detect-todo"):
        return 2
    ...
    payload = predicates.detect_state(issue_number, 'todo')
