...
def main() -> int:
    parser = argparse.ArgumentParser(description="Detect whether an issue is in the 'done' lifecycle state.")
    ...
    if not bootstrap_gate.enforce("detect-done"):
        return 2
    ...
    payload = predicates.detect_state(issue_number, 'done')
