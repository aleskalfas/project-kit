...
def main() -> int:
    ...
    if not bootstrap_gate.enforce("enable-default-agent", capability_root=capability_root):
        return 2
    ...
    # Foreign-repo mutation guard (COR-039 / ADR-034). The live overlay file and
    # the merge-settings.sh run target project_root, which is resolved from the
    # cwd-walk in resolve_capability_root, so a `cd /B` redirects this write into
    # a DIFFERENT repo than the session anchor (CLAUDE_PROJECT_DIR); gate that
    # unless the operator confirms the override. Runs before any write.
