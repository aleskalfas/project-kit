...
- The naming also sidesteps a same-repo collision with the methodology's COR-016 *scripted-scenario* storyboards (a different sense of "storyboard"): the recording capability's storyboard-authoring lives as the `storyboard-author` sub-procedure of the `demo-recording` composite skill, namespaced under it — which keeps the two senses distinct from the methodology's core `storyboard-author` skill.
