...
The precise description of how a command produces output under this capability: the event model, the vocabulary, the renderer contract, selection, and the invariants. This is the **evolving reference** ("what-is") governed by the architectural decision in [ADR-053](../../../docs/architecture/decisions/ADR-053-cli-output-discipline.md) ("what + why"). The design rationale and prior-art survey live in the scratchpad note `../../scratchpad/active/2026-08-27-cli-communication-conventions.md`.
...
The reference binding's existing semantic-data parts (`title()` / `status()` / `section()`) are the first events; the discipline adds the `type` discriminator and a per-type payload schema. _(Migration planned — ADR-053 sequencing.)_
...
The per-language-binding capability sub-shape (COR-017 extension) is authored when a second-language consumer appears (ADR-053 Decision 6).
