...
| `methodology-reviewer` | Once the artifact is authored — runs the disciplines audit (axiom / project-neutrality / principles-not-inventory / universal applicability / artifact-role placement). **Also a required merge-gate reviewer on every PR** (#788), so its `CHANGES_REQUESTED` blocks the merge; invoking it by hand *before* opening the PR is how you avoid meeting its findings at the gate. | Read-only. |
