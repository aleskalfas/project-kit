...
- **`.pkit/agents/README.md`** — agents area spec. Five universal core agents shipped today: `critic` (adversarial pre-proposal review per COR-024), `architect` (architectural custodian + ADR custody per COR-024 + COR-025), `methodology-reviewer` (disciplines audit on artifacts), `convention-compliance-reviewer` (diff checks against conventions), and `process-author` (the predicates behind a process definition's declared evaluables — COR-044's teeth tier; the first *producing* agent at core, owner-scoped via the `process-authoring-targets` overlay category, which ships empty so it owns nothing until you grant it). Deployed by `.pkit/adapters/claude-code/deploy-agents.sh`. The project-management capability ships `project-manager` (file/validate/transition issues + autonomous batch-planning) per [COR-026](.pkit/decisions/core/COR-026-agent-placement-by-discipline.md) + [project-management:DEC-029-project-manager-agent-shape]; install the capability to use it.
...
- **Versioning (PRJ-002).** Declared per-PR, applied release-driven, written main-only. A PR that lands a *surface change* (something an adopter could observe, depend on, or break against) **declares** it by adding a changeset file under `.changes/unreleased/` (`changie new`, or hand-write the YAML: `component` + `kind` segment + note) — and **never** edits `.pkit/VERSION`, a component's `version`, or `requires_backbone` (D1). The segment (`patch`/`minor`/`major`/`none`) is a human judgment of surface impact, not a function of the commit type (D2). Version numbers and `requires_backbone` broadening are written only on `main` by the release step, which consumes pending changesets (D3/D4). Non-surface work (docs refinement, internal refactors, behaviour-preserving fixes, test additions, PRJ records) needs no changeset. Do **not** run `pkit version bump` on a branch.
...
**Dispatch reviewers without a `name`.** When invoking any of these via the `Agent` tool, do not pass a `name`. With Claude Code's experimental agent-teams feature enabled, a named spawn becomes a persistent teammate and its result arrives **hard-truncated at 4064 characters** (marker: `[result truncated — ask the agent for the rest via SendMessage]`), with the rest reachable only via `SendMessage` — which these agents' tool grants do not include. Reviews run 8–12 KB, so most of the critique is lost *and the surviving prefix reads as complete*. Measured on three named teammates in one session. You can detect the condition rather than remember it: if the `Agent` tool exposes a `name` parameter, teams is on; if not, naming is unavailable and the hazard is unreachable. Standing discipline, not interim: a harness-level check can detect that teams is on but cannot stop a dispatcher naming a spawn — only the dispatcher can. Recorded in DEC-029's dispatch-discipline paragraph; the requirement it protects is `delegated-result-complete` (COR-047).
