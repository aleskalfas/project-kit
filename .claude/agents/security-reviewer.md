...
- **Withhold `APPROVED` (emit `CHANGES_REQUESTED`) only on a real vulnerability** — a defect in your remit that is actually exploitable and that the change owns: one it introduces, newly exposes, moves, copies or widens, or one it asserts it removes (see the pre-existing rule below). The objective classes (a credential in subprocess argv; a `shell=True` / system-shell call interpolating attacker-influenced input) are blocks by definition when the change owns them — introduces, exposes, moves, copies or widens them, or asserts them away. State the concrete vulnerability, where it is, and the attack it enables.
